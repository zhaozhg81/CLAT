                                        #' This function runs the simulation and plots the figure for case II.
                                        #'
...
caseII <- function(beta, numSim=1000)
{
  n <- 5000
  ...
  for (l in 1:rNum){
    for (k in 1:numSim){
      ...
      X[1:(n-NONzero)] <- rt(n-NONzero,df)
      X[(n-NONzero+1):(n-NONzero + floor(NONzero * p1) )  ] <- sigma * rt( floor(NONzero*p1), df) + mu[l]
      X[(n-NONzero+1+ floor(NONzero * p1) ) : n  ] <- sigma * rt( NONzero - floor(NONzero*p1) , df) - mu[l]
      
      ## Transform to normal case
      X <- qnorm( pt( X, df, lower.tail=FALSE), lower.tail=FALSE )
      f0x <- dnorm(X)
      f1x <- p1 * dt( ( qt( pnorm(X, lower.tail=FALSE), df, lower.tail=FALSE) - mu[l] )/sigma ,df )/sigma /  dt( qt( pnorm(X, lower.tail=FALSE), df, lower.tail=FALSE) ,df ) * dnorm(X) +
        (1-p1) * dt( ( qt( pnorm(X, lower.tail=FALSE), df, lower.tail=FALSE) + mu[l] )/sigma, df )/sigma /  dt( qt( pnorm(X, lower.tail=FALSE), df, lower.tail=FALSE), df ) * dnorm(X)
  ...
  plot(x, y$FNRBH, type="b", col="grey", pch=25, ylim=c(0,max( y$FNR, y$FNRBH, y$FNRlocfdr, y$FNRlocfdr.em, y$FNRSunCai, y$FNRoracle)), xlab=TeX("$\\mu$"), ylab="", main="mFNR", cex=CEX, lwd=LWD )
